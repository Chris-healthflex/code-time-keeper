  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    if (error) {
      if (error.message.toLowerCase().includes("not found") || error.status === 400) {
        setOtpError("This email hasn't been invited as an admin.");
      } else {
    ...
    <main className="relative flex min-h-screen flex-col items-center justify-center bg-background px-6">
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="mb-10 text-center">
          ...
          <h1 className="mt-6 text-2xl font-medium tracking-tight text-foreground">Admin Login</h1>
          <p className="mt-2 text-[13px] text-muted-foreground">
            Sign in to access your Stance Health admin dashboard.
          </p>
        </div>
